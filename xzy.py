...
async def ensure_tailscaled(container_name):
    """Install/start Tailscale inside a VPS container, with Docker-safe flags."""
...
if ! pgrep -x tailscaled >/dev/null 2>&1; then
    ...
    tail -n 50 /tmp/tailscaled.log 2>/dev/null || true
    exit 1
fi
"""
...
@is_admin()
async def edit_vps(ctx, user: discord.Member, vps_number: int, ram: int, cpu: int, disk: Optional[int] = None):
    """Update another user's VPS resources (Admin only)."""
    user_id = str(user.id)
    vps_list = vps_data.get(user_id, [])
    ...
    if ram <= 0 or cpu <= 0 or (disk is not None and disk <= 0):
        await ctx.send(embed=create_error_embed(
            "Invalid Specs",
            "RAM, CPU and Disk must be positive integers."
        ))
    ...
    if not container_name:
        await ctx.send(embed=create_error_embed("Invalid VPS", "This VPS has no container name."))
        return

    await ctx.send(embed=create_info_embed(
        "Updating VPS",
        f"Updating `{container_name}` for {user.mention}..."
    ))

    try:
        await execute_docker(
            f"docker update --memory={ram}g --cpus={cpu} {container_name}",
            timeout=60
        )
    except Exception as e:
        await ctx.send(embed=create_error_embed("Update Failed", str(e)))
        return
    ...
    storage_text = (
        f"{vps.get('storage', 'Unknown')} (record updated; no Docker quota is configured)"
        if disk is not None else vps.get("storage", "Unknown")
    )
    embed = create_success_embed("VPS Updated Successfully")
    embed.add_field(name="Owner", value=user.mention, inline=True)
    embed.add_field(name="VPS ID", value=f"#{vps_number}", inline=True)
