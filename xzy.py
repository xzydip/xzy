...
@is_admin()
async def renew_vps(ctx, user: discord.Member, vps_number: int, days: int = 30):
    """Admin renew an expired/stopped VPS by restarting it and assigning a new expiry date."""
    if days <= 0:
        await ctx.send(embed=create_error_embed("Invalid Expiry", "Renew days must be a positive integer."))
        return
    ...
    await ctx.send(embed=create_success_embed(
        "VPS Renewed",
        f"✅ {user.mention}'s **VPS #{vps_number}** (`{container_name}`) has been renewed for **{days} days**."
    ))

    try:
        await user.send(embed=create_success_embed(
            "✅ VPS Renewed",
            f"Your VPS `{container_name}` has been renewed by an admin for **{days} days**."
            f"\nNew expiry: `{new_exp[:10]}`"
        ))
    except discord.Forbidden:
        pass
