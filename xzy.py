...
def get_storage_usage():
    try:
        ...
        used = parts[2]
        available = parts[3]
        usage_pct = float(parts[4].rstrip('%')) if parts[4].endswith('%') else 0.0
        if total.endswith('G'):
            total_val = float(total[:-1])
        elif total.endswith('M'):
            total_val = float(total[:-1]) / 1024
        elif total.endswith('T'):
            total_val = float(total[:-1]) * 1024
        else:
            total_val = float(total) / 1024
        if used.endswith('G'):
            used_val = float(used[:-1])
        elif used.endswith('M'):
            used_val = float(used[:-1]) / 1024
        elif used.endswith('T'):
            used_val = float(used[:-1]) * 1024
        else:
            used_val = float(used) / 1024
        available_val = float(available[:-1]) if available.endswith('G') else float(available[:-1]) / 1024 if available.endswith('M') else float(available[:-1]) * 1024 if available.endswith('T') else float(available) / 1024
        return used_val, total_val, usage_pct
    except Exception as e:
...
def get_cpu_cores_usage():
    try:
        result = subprocess.run(['top', '-bn1'], capture_output=True, text=True, check=False)
        cpu_lines = [line for line in result.stdout.splitlines() if '%Cpu(s):' in line]
        if not cpu_lines:
            return 0.0, 0.0, 0.0
        line = cpu_lines[0]
        for part in line.split(','):
            if 'us,' in part:
                user = float(part.split('%')[0].strip())
            elif 'sy,' in part:
                system = float(part.split('%')[0].strip())
            elif 'id,' in part:
                idle = float(part.split('%')[0].strip())
        used = user + system
        return used, 100.0, used
    except Exception as e:
        logger.error(f"Error getting CPU cores usage: {e}")
        return 0.0, 0.0, 0.0
...
def ram_monitor():
    global ram_monitor_active
    while ram_monitor_active:
        try:
            _, _, ram_usage = get_ram_usage()
            logger.info(f"Current RAM usage: {ram_usage}%")
            if ram_usage > RAM_THRESHOLD:
                logger.warning(f"RAM usage ({ram_usage}%) exceeded threshold. Stopping all containers.")
                try:
                    subprocess.run(['docker', 'stop', '--time=5'] +
                                   [vps['container_name']
                                    for vps_list in vps_data.values()
                    ...
                    logger.error(f"Error stopping containers due to RAM threshold: {e}")
            time.sleep(CHECK_INTERVAL)
        except Exception as e:
...
cpu_thread.start()
ram_thread = threading.Thread(target=ram_monitor, daemon=True)
ram_thread.start()
...
@is_admin()
async def vps_info(ctx, user: discord.Member = None, vps_number: int = None, container_name: str = None):
    """Get VPS information by user, or by user + number, or by container name."""

    async def send_vps_embed(found_user, found_vps):
        container = found_vps.get('container_name', 'Unknown')
        embed = create_embed(f"🖥️ VPS - {container}", f"Owned by {found_user.mention}", 0x1a1a1a)
        embed.add_field(
            name="Specs",
            value=(
                f"**Plan:** {found_vps.get('plan', 'Custom')}\n"
                f"**RAM:** {found_vps.get('ram', 'Unknown')}\n"
                f"**CPU:** {found_vps.get('cpu', 'Unknown')} Cores\n"
    ...
    if user is not None and vps_number is None and container_name is None:
        user_id = str(user.id)
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            await ctx.send(embed=create_error_embed("Not Found", f"{user.mention} has no VPS."))
            return

        embed = create_embed(f"🖥️ VPS Information - {user.name}", f"Total: {len(vps_list)} VPS", 0x1a1a1a)
        for i, vps in enumerate(vps_list, start=1):
            embed.add_field(
                name=f"VPS #{i} - `{vps.get('container_name', 'Unknown')}`",
                value=(
                    f"**Plan:** {vps.get('plan', 'Custom')}\n"
                    f"**RAM:** {vps.get('ram', 'Unknown')}\n"
                    f"**CPU:** {vps.get('cpu', 'Unknown')} Cores\n"
                    f"**Storage:** {get_vps_storage(vps)}\n"
    ...
    if user is not None and vps_number is not None:
        user_id = str(user.id)
        vps_list = vps_data.get(user_id, [])
        if not vps_list or vps_number < 1 or vps_number > len(vps_list):
            ...
            return
        found_vps = vps_list[vps_number - 1]
        await send_vps_embed(user, found_vps)
        return
    ...
    if container_name:
        ...
        if not found_vps:
            ...
            return
        await send_vps_embed(found_user, found_vps)
        return
...
@bot.hybrid_command(name='system-usage')
@is_admin()
async def system_usage(ctx):
    """Show current system RAM, CPU, and storage usage."""
    ram_used, ram_total, ram_pct = get_ram_usage()
    cpu_used, _, _ = get_cpu_cores_usage()
    storage_used, storage_total, storage_pct = get_storage_usage()

    if ram_total > 0:
        ram_left = ram_total - ram_used
    else:
        ram_left = 0

    if storage_total > 0:
        storage_left = storage_total - storage_used
    else:
        storage_left = 0

    embed = create_embed("🖥️ System Usage", "Live server resource summary", 0x00ccff)
    embed.add_field(
        name="💾 RAM",
        value=(
            f"Used: **{ram_used:.1f} MB**\n"
            f"Total: **{ram_total:.1f} MB**\n"
            f"Available: **{ram_left:.1f} MB**\n"
            f"Usage: **{ram_pct:.1f}%**"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="💽 Storage",
        value=(
            f"Used: **{storage_used:.1f} GB**\n"
            f"Total: **{storage_total:.1f} GB**\n"
            f"Free: **{storage_left:.1f} GB**\n"
            f"Usage: **{storage_pct:.1f}%**"
        ),
        inline=False
    )
    await ctx.send(embed=embed)
...
def build_help_pages(is_user_admin, is_user_main_admin):
    ...
    credits_embed.add_field(name="💳 Commands", value=(
        "Contact the admin for plan details."
    ), inline=False)
    ...
    admin_embed.add_field(name="📊 Info & Economy", value=(
            ...
            "`/system-usage` — Live RAM, CPU, and storage summary\n"
