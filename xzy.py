...
If someone asks:
        lxcfs_meminfo_mount = ""
        if os.path.exists("/var/lib/lxcfs/proc/meminfo"):
            lxcfs_meminfo_mount = (
                "--mount type=bind,source=/var/lib/lxcfs/proc/meminfo," 
                "target=/proc/meminfo,readonly "
            )
        run_cmd = (
            f"docker run -d "
...
def create_embed(title, description="", color=0x1a1a1a, fields=None):
    ...
    embed.set_footer(
        text=f"Xzy Hosting | VPS Manager • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        icon_url=""
    )
    return embed
...
async def get_running_vps_count():
    """Return the number of saved VPS containers that are actually running."""
    try:
        output = await execute_docker(
            "docker ps --filter status=running --format '{{.Names}}'",
            timeout=15
        )
        running_names = set(output.splitlines()) if isinstance(output, str) else set()
    except Exception as error:
        logger.warning(f"Unable to read live Docker VPS status: {error}")
        return sum(
            1
            for vps_list in vps_data.values()
            for vps in vps_list
    ...
    for vps_list in vps_data.values():
        for vps in vps_list:
            is_running = vps.get("container_name") in running_names
            new_status = "running" if is_running else "stopped"
            if vps.get("status") != new_status:
...
async def refresh_vps_presence():
    ...
    running_vps = await get_running_vps_count()
    try:
        await bot.change_presence(
            status=discord.Status.dnd,
            activity=discord.Activity(
                type=discord.ActivityType.watching,
                name=f"🚀 VPS {running_vps}-Node Cluster: Online"
            )
        )
        logger.info(f"Presence updated: {running_vps} VPS Instances Running (DND)")
...
async def update_vps_presence():
    """Keep the bot presence updated with the current running VPS count."""
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if action == 'reinstall':
            ...
            try:
                ...
                sshx_embed.set_footer(text=f"Powered by Xzy Hosting • Premium VPS Management • {now}")
                ...
                try:
                    ...
                    await interaction.followup.send(
                        embed=sshx_embed,
                        content="DMs are disabled, so your temporary SSHX link is shown here privately.",
                        ephemeral=True)
                    await interaction.followup.send(
                        embed=create_info_embed("DM Unavailable", "Enable DMs if you want future SSHX links delivered privately there."),
                        ephemeral=True)
            except Exception as e:
                await interaction.followup.send(
...
class CreateVPSView(discord.ui.View):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        try:
            ...
            server_embed.set_footer(text=f"Xzy Hosting | Op Big Node • {display_time}")
            ...
            dm_embed.add_field(
                name="⌯⌲ Management",
                value=(
                    "• Use `/manage` to start/stop/reinstall your VPS\n"
                    "• Use `/manage` → SSH/SSHX for terminal access\n"
                    "• Contact admin for upgrades or issues"
                ),
                inline=False
            )
            dm_embed.set_footer(text=f"Powered by Xzy Hosting • Premium VPS Management • {display_time}")
            try:
...
async def buy_with_credits(ctx, plan: str, processor: str = "Intel"):
    ...
    try:
        ...
        embed.set_footer(text=f"Xzy Hosting | Ip Big Node • {display_time}")
        await ctx.send(embed=embed)
...
# Typo aliases
@bot.hybrid_command(name='mangage')
