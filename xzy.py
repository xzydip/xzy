...
async def refresh_live_vps_status():
    """Synchronize saved VPS status with the current Docker container state."""
    changed = False
    for vps_list in vps_data.values():
        for vps in vps_list:
            ...
            try:
                ...
                live_state = stdout.decode().strip() if proc.returncode == 0 else ""
            except (asyncio.TimeoutError, OSError):
                live_state = ""

            live_status = "running" if live_state == "running" else "stopped"
            if vps.get("status") != live_status:
