...
def load_vps_data():
    try:
        with open(os.path.join(DATA_DIR, 'vps_data.json'), 'r') as f:
            loaded = json.load(f)
            vps_data = {}
...
def load_admin_data():
    try:
        with open(os.path.join(DATA_DIR, 'admin_data.json'), 'r') as f:
            return json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
...
DEFAULT_PLAN_DATA = {
    "Paid": {"ram": 4, "cpu": 1, "storage": 20, "price": 150},
    "1 Boost": {"ram": 8, "cpu": 1, "storage": 40, "price": 0},
    "2 Boost": {"ram": 12, "cpu": 2, "storage": 80, "price": 0},
    "Paid 8GB": {"ram": 8, "cpu": 2, "storage": 40, "price": 250},
    "Paid 12GB": {"ram": 12, "cpu": 2, "storage": 80, "price": 400},
    "Paid 16GB": {"ram": 16, "cpu": 3, "storage": 120, "price": 700},
}


def load_plan_data():
    try:
        with open(os.path.join(DATA_DIR, 'plan_data.json'), 'r') as f:
            loaded = json.load(f)
            if isinstance(loaded, dict) and loaded:
                return loaded
            raise ValueError("plan_data.json is empty")
    except (FileNotFoundError, json.JSONDecodeError, ValueError):
        logger.warning("plan_data.json not found or corrupted, initializing with default plans")
...
def save_data():
    try:
        ...
        with open(os.path.join(DATA_DIR, 'vps_data.json'), 'w') as f:
            json.dump(vps_data, f, indent=4)
        with open(os.path.join(DATA_DIR, 'admin_data.json'), 'w') as f:
            json.dump(admin_data, f, indent=4)
        with open(os.path.join(DATA_DIR, 'plan_data.json'), 'w') as f:
...
async def sync_vps_statuses():
    """Reconcile saved VPS statuses with the current Docker container state."""
    records = [
        vps
        for vps_list in vps_data.values()
        for vps in vps_list
    ...
    async def inspect_vps(vps):
        try:
            state = await execute_docker(
                f"docker inspect -f '{{{{.State.Status}}}}' {vps['container_name']}",
                timeout=10,
            )
            vps["status"] = "running" if str(state).strip() == "running" else "stopped"
        except Exception:
            # Keep the saved value if Docker is temporarily unavailable.
            pass

    await asyncio.gather(*(inspect_vps(vps) for vps in records))
...
async def update_vps_presence():
    ...
    try:
        ...
        # A temporary Docker or Discord failure must not stop future updates.
        logger.warning(f"VPS presence refresh failed: {error}")
...
def get_vps_storage(vps):
    """Return the configured storage, including a fallback for old records."""
    ...
    if vps.get("plan") in plan_storage:
        return plan_storage[vps["plan"]]
    return "30GB"
...
class CreateVPSView(discord.ui.View):
    ...
    def __init__(self, ctx, user, ram, cpu, disk, expiry_days=0, plan=None):
        super().__init__(timeout=300)
        self.ctx = ctx
        self.user = user
        self.ram = ram
...
async def manage_vps(ctx, user: discord.Member = None):
    ...
    if user:
        ...
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            embed = create_embed("No VPS Found", "", 0xff3366)
            embed.add_field(name="Quick Actions", value="• `/plans` - View plans", inline=False)
            await ctx.send(embed=embed)
            return
...
async def list_all_vps(ctx):
    ...
    embed.add_field(
        ...
        value=f"**Total Users:** {sum(1 for vps_list in vps_data.values() if vps_list)}\n**Total VPS:** {total_vps}\n**Running:** {running_vps}\n**Stopped:** {stopped_vps}",
        inline=False
    )
    if user_summary:
...
@bot.hybrid_command(name='plan')
@bot.hybrid_command(name='addplan')
@is_admin()
async def add_plan(ctx, plan_name: str, ram: int, cpu: int, storage: int, price: int = 0):
    """Create a new VPS plan.
    Example: /plan "Elite 8GB" 8 2 80 350
    """
    name = plan_name.strip()
    if not name:
        await ctx.send(embed=create_error_embed("Invalid Plan", "Plan name cannot be empty."))
        return
    if ram <= 0:
        await ctx.send(embed=create_error_embed("Invalid RAM", "RAM must be a positive integer."))
        return
    if cpu <= 0:
        ...
        return
    if storage <= 0:
        await ctx.send(embed=create_error_embed("Invalid Storage", "Storage must be a positive integer."))
        return
    if price < 0:
        await ctx.send(embed=create_error_embed("Invalid Price", "Price must be zero or a positive integer."))
        return
    if name in plan_data:
        await ctx.send(embed=create_error_embed("Duplicate Plan", f"A plan named `{name}` already exists. Use `/editplan` to update it."))
        return

    plan_data[name] = {"ram": ram, "cpu": cpu, "storage": storage, "price": price}
    save_data()
    embed = create_success_embed("Plan Added", f"New plan `{name}` has been created successfully.")
...
@bot.hybrid_command(name='deleteplan')
@is_admin()
async def delete_plan(ctx, plan_name: str):
    """Delete a VPS plan from the list."""
    name = plan_name.strip()
    if not name:
        await ctx.send(embed=create_error_embed("Invalid Plan", "Plan name cannot be empty."))
        return
    if name not in plan_data:
        await ctx.send(embed=create_error_embed("Plan Not Found", f"Plan `{name}` does not exist."))
        return

    del plan_data[name]
    save_data()
    await ctx.send(embed=create_success_embed("Plan Deleted", f"Plan `{name}` has been removed successfully."))


@bot.hybrid_command(name='editplan')
@is_admin()
async def edit_plan(ctx, plan_name: str, ram: Optional[int] = None, cpu: Optional[int] = None, storage: Optional[int] = None, price: Optional[int] = None, new_name: Optional[str] = None):
    """Admin command to edit or create a VPS plan.
    Example: /editplan "Paid 8GB" 8 2 40 250
    To rename: /editplan "Paid 8GB" 8 2 40 250 "Custom Plan"
    """
    name = plan_name.strip()
    if not name:
        await ctx.send(embed=create_error_embed("Invalid Plan", "Plan name cannot be empty."))
    ...
    if name not in plan_data and (ram is None and cpu is None and storage is None and price is None):
        await ctx.send(embed=create_error_embed("Plan Not Found", f"Plan `{name}` does not exist. Provide values to create it."))
        return

    if new_name is not None:
        new_key = new_name.strip()
        if not new_key:
            await ctx.send(embed=create_error_embed("Invalid Name", "New plan name cannot be empty."))
            return
        if new_key in plan_data and new_key != name:
            await ctx.send(embed=create_error_embed("Duplicate Plan", f"A plan named `{new_key}` already exists."))
            return

    if name not in plan_data:
        plan_data[name] = {"ram": 0, "cpu": 0, "storage": 0, "price": 0}
    ...
    if new_name is not None:
        final_name = new_key
        if final_name != name:
            plan_data[final_name] = plan_data.pop(name)

    save_data()
    embed = create_success_embed("Plan Updated", f"Plan `{final_name}` has been updated successfully.")
    embed.add_field(name="RAM", value=f"{plan_data[final_name].get('ram', 0)}GB", inline=True)
    embed.add_field(name="CPU", value=f"{plan_data[final_name].get('cpu', 0)} Core(s)", inline=True)
    embed.add_field(name="Storage", value=f"{plan_data[final_name].get('storage', 0)}GB", inline=True)
    embed.add_field(name="Price", value=f"₹{plan_data[final_name].get('price', 0)}", inline=True)
    await ctx.send(embed=embed)
...
async def show_plans(ctx):
    ...
    if not plan_data:
        embed.add_field(name="No Plans Found", value="Use `/editplan` to create a plan.", inline=False)
    else:
        for name, details in plan_data.items():
            ram = details.get("ram", 0)
            cpu = details.get("cpu", 0)
            storage = details.get("storage", 0)
            price = details.get("price", 0)
            price_text = f"**Price:** ₹{price}" if price and price > 0 else "**Price:** N/A"
            embed.add_field(
                name=name,
                value=f"**RAM:** {ram}GB\n**CPU:** {cpu} Core(s)\n**Storage:** {storage}GB\n{price_text}",
                inline=True
            )
    embed.add_field(name="How to Buy", value="Use `/pay` for payment information", inline=False)
    await ctx.send(embed=embed)
...
async def vps_info(ctx, container_name: str = None):
    ...
    if not container_name:
            ...
            try:
                ...
                for i, v in enumerate(vl):
                    all_vps.append(
                        ...
                        f"{v.get('status','?').upper()} - Storage: {get_vps_storage(v)} - "
                        f"Expiry: {format_expiry(v.get('expires', 'Never'))}"
                    )
            except:
                pass
        ...
        embed.add_field(
            name="Specs",
            value=(
                f"**Plan:** {found_vps.get('plan', 'Custom')}\n"
                f"**RAM:** {found_vps['ram']}\n"
                f"**CPU:** {found_vps['cpu']} Cores\n"
                f"**Storage:** {get_vps_storage(found_vps)}"
            ),
            inline=True
        )
...
def build_help_pages(is_user_admin, is_user_main_admin):
    ...
    credits_embed.add_field(name="📦 Available Plans", value=(
        "**Paid** — 4GB RAM | 1 CPU | 20GB | ₹150\n"
        "**Paid** — 8GB RAM | 2 CPU | 40GB | ₹250\n"
        "**Paid** — 12GB RAM | 2 CPU | 80GB | ₹400\n"
        "**Paid** — 16GB RAM | 3 CPU | 120GB | ₹700\n"
        "**1 Boost** — 8GB RAM | 1 CPU | 40GB\n"
        "**2 Boost** — 12GB RAM | 2 CPU | 80GB"
    ), inline=False)
...
async def my_info(ctx):
    ...
    if vps_list:
        ...
        embed.add_field(name="🖥️ Your VPS", value="No VPS yet. Contact an admin to request one.", inline=False)
    await ctx.send(embed=embed)
