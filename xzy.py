...
async def create_docker_container(container_name, ram_mb, cpu_count, ssh_port, password, disk_gb=30, image=None):
    ...
    if inspect not in (True, "", None):
        ...
        run_cmd = (
            ...
            f"--tmpfs /run "
            f"--tmpfs /run/lock "
            f"--volume /sys/fs/cgroup:/sys/fs/cgroup:rw "
    ...
    if image.startswith(("ubuntu:", "debian:")):
        setup_script = (
            "export DEBIAN_FRONTEND=noninteractive; "
            "apt-get update -qq && "
            "apt-get install -y openssh-server tmate curl docker.io systemd systemd-sysv dbus ca-certificates gnupg -qq && "
            "if ! command -v playit >/dev/null 2>&1; then "
            "  curl -SsL https://playit-cloud.github.io/ppa/setup.deb.sh | bash && "
            "  apt-get install -y playit -qq; "
            "fi; "
            ...
            "systemctl enable playit 2>/dev/null || true; "
            "/usr/sbin/sshd"
        )
    else:
        ...
        setup_script = (
            ...
            "  dnf -y install openssh-server tmate curl docker systemd systemd-udev dbus ca-certificates; "
            "else "
            "  yum -y install openssh-server tmate curl docker systemd dbus ca-certificates; "
            "fi; "
            "mkdir -p /var/run/sshd /run/sshd; "
            "echo 'PermitRootLogin yes' >> /etc/ssh/sshd_config; "
            "echo 'PasswordAuthentication yes' >> /etc/ssh/sshd_config; "
            ...
            "    *) echo 'Unsupported architecture for Playit agent' >&2; exit 1 ;; "
            "  esac; "
            "  curl -fsSL \"https://github.com/playit-cloud/playit-agent/releases/latest/download/playit-linux-$playit_arch\" -o /usr/local/bin/playit; "
            "  chmod 755 /usr/local/bin/playit; "
            "  printf '%s\\n' '[Unit]' 'Description=playit.gg tunnel agent' 'After=network-online.target' 'Wants=network-online.target' '' '[Service]' 'ExecStart=/usr/local/bin/playit' 'Restart=on-failure' '' '[Install]' 'WantedBy=multi-user.target' > /etc/systemd/system/playit.service; "
            "fi; "
