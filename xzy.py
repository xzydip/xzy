...
async def on_ready():
    ...
    await bot.change_presence(
        ...
        activity=discord.Activity(type=discord.ActivityType.watching, name="Xzy Hosting | VPS Manager")
    )
...
async def ports_help(ctx):
    ...
    embed.add_field(
        name="⌯⌲ Commands",
        value=(
            "**`1ports`** - Port forwarding help\n"
            "**`1pcreate <vps_id> <port>`** - Add port forward\n"
            "**`1pcreate <vps_id>`** - Add random 2-digit port\n"
            "**`1ports list`** - List your port forwards\n"
            "**`1ports remove <id>`** - Remove port forward"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="⌯⌲ Navigation",
        value=(
            "• Use `1pcreate <vps_id>` for a random 2-digit port\n"
            "• Use `1pcreate <vps_id> <port>` for a specific port\n"
            "• Prefix: `1`"
        ),
        inline=False
    )
...
def build_help_pages(is_user_admin, is_user_main_admin):
    ...
    user_embed.add_field(
        ...
        value=(
            "`1ports` — Port forwarding help\n"
            "`1pcreate <vps_id> <port>` — Create a port forward\n"
            "`1pcreate <vps_id>` — Create random 2-digit port\n"
    ...
    port_embed.add_field(
        ...
        value=(
            "**`1ports`** - Port forwarding help\n"
            "**`1pcreate <vps_num> <port>`** - Add port forward\n"
            "**`1pcreate <vps_num>`** - Random 2–4 digit port\n"
            "**`1ports list`** - List your port forwards\n"
            "**`1ports remove <id>`** - Remove port forward"
        ),
        inline=False
    ...
    if is_user_admin:
        ...
        admin_embed.add_field(name="📊 Info & Economy", value=(
            ...
            "`1msg select chenel #channel msg <message>` — Send exact message to a channel\n"
...
async def admin_channel_message(ctx, *, raw: str = ""):
    ...
    Usage:
    1msg select chenel #channel msg Your message
    """
...
async def on_message(message):
    ...
    # Existing command system stays untouched.
