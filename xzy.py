...
async def create_docker_container(container_name, ram_mb, cpu_count, ssh_port, password, disk_gb=30, image=None):
    ...
    if image.startswith(("ubuntu:", "debian:")):
        setup_script = (
            "export DEBIAN_FRONTEND=noninteractive; "
            "apt-get update -qq && "
            "apt-get install -y openssh-server tmate curl ca-certificates gnupg systemd systemd-sysv docker.io -qq && "
            "install -d -m 0755 /etc/apt/keyrings; "
            "curl -fsSL https://playit-cloud.github.io/ppa/key.gpg | gpg --dearmor -o /etc/apt/keyrings/playit.gpg; "
            "echo 'deb [signed-by=/etc/apt/keyrings/playit.gpg] https://playit-cloud.github.io/ppa stable main' > /etc/apt/sources.list.d/playit.list; "
            "apt-get update -qq && apt-get install -y playit -qq; "
            "mkdir -p /var/run/sshd /run/sshd && "
            "echo 'PermitRootLogin yes' >> /etc/ssh/sshd_config && "
            "echo 'PasswordAuthentication yes' >> /etc/ssh/sshd_config && "
            f"echo 'root:{password}' | chpasswd && "
            "/usr/sbin/sshd && "
            "systemctl enable playit.service && systemctl start playit.service || true"
        )
    else:
        ...
        setup_script = (
            ...
            "  dnf -y install openssh-server tmate curl ca-certificates systemd docker; "
            "else "
            "  yum -y install openssh-server tmate curl ca-certificates systemd docker; "
            "fi; "
            ...
            "systemctl daemon-reload; systemctl enable playit.service; "
            "mkdir -p /var/run/sshd /run/sshd; "
            "echo 'PermitRootLogin yes' >> /etc/ssh/sshd_config; "
            "echo 'PasswordAuthentication yes' >> /etc/ssh/sshd_config; "
            f"echo 'root:{password}' | chpasswd; "
            "/usr/sbin/sshd; systemctl start playit.service || true"
        )
